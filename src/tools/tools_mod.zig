...
fn AgentToolsImpl(comptime ClientType: type) type {
    ...
    return struct {
        ...
        fn buildStandardObservedTemplatesAlloc(
            ...
            if (std.mem.eql(u8, opcode_name, "jetton_provide_wallet_address")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard jetton_provide_wallet_address @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> jetton_provide_wallet_address @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
                    .example_spec_json = null,
                    .note = try allocator.dupe(u8, "Spec JSON: {\"query_id\":0,\"owner_address\":\"0:...\",\"include_address\":true}"),
                };
            }

            if (std.mem.eql(u8, opcode_name, "jetton_take_wallet_address")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard jetton_take_wallet_address @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> jetton_take_wallet_address @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
                    .example_spec_json = null,
                    .note = try allocator.dupe(u8, "Spec JSON: {\"query_id\":0,\"wallet_address\":\"0:...\",\"owner_address\":\"0:...\"}"),
                };
            }
            ...
            if (std.mem.eql(u8, opcode_name, "nft_get_static_data")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard nft_get_static_data @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> nft_get_static_data @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
            ...
            if (std.mem.eql(u8, opcode_name, "nft_report_static_data")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard nft_report_static_data @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> nft_report_static_data @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
                    .example_spec_json = null,
                    .note = try allocator.dupe(u8, "Spec JSON: {\"query_id\":0,\"index\":\"0x0\",\"collection\":\"0:...\"}"),
                };
            }

            if (std.mem.eql(u8, opcode_name, "nft_get_royalty_params")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard nft_get_royalty_params @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> nft_get_royalty_params @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
            ...
            if (std.mem.eql(u8, opcode_name, "nft_report_royalty_params")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard nft_report_royalty_params @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> nft_report_royalty_params @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
                    .example_spec_json = null,
                    .note = try allocator.dupe(u8, "Spec JSON: {\"query_id\":0,\"numerator\":0,\"denominator\":1,\"destination\":\"0:...\"}"),
                };
            }

            if (std.mem.eql(u8, opcode_name, "sbt_prove_ownership")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard sbt_prove_ownership @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> sbt_prove_ownership @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
                    .example_spec_json = null,
                    .note = try allocator.dupe(u8, "Spec JSON: {\"query_id\":0,\"destination\":\"0:...\",\"forward_comment\":\"<text>\",\"with_content\":true}"),
                };
            }

            if (std.mem.eql(u8, opcode_name, "sbt_request_owner")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard sbt_request_owner @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> sbt_request_owner @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
                    .example_spec_json = null,
                    .note = try allocator.dupe(u8, "Spec JSON: {\"query_id\":0,\"destination\":\"0:...\",\"forward_comment\":\"<text>\",\"with_content\":false}"),
                };
            }

            if (std.mem.eql(u8, opcode_name, "sbt_destroy")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard sbt_destroy @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> sbt_destroy @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
            ...
            if (std.mem.eql(u8, opcode_name, "sbt_revoke")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard sbt_revoke @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> sbt_revoke @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
            ...
            if (std.mem.eql(u8, opcode_name, "sbt_ownership_proof")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard sbt_ownership_proof @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> sbt_ownership_proof @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
                    .example_spec_json = null,
                    .note = try allocator.dupe(u8, "Spec JSON: {\"query_id\":0,\"item_id\":\"0x0\",\"owner\":\"0:...\",\"data_comment\":\"<text>\",\"revoked_at\":0}"),
                };
            }

            if (std.mem.eql(u8, opcode_name, "sbt_owner_info")) {
                return .{
                    .body_cli_template = try allocator.dupe(u8, "ton-zig-agent-kit cell build-standard sbt_owner_info @spec.json"),
                    .send_cli_template = if (is_incoming)
                        try std.fmt.allocPrint(
                            allocator,
                            "ton-zig-agent-kit wallet send-standard <wallet_addr> {s} <amount_nanoton> sbt_owner_info @spec.json",
                            .{contract_address},
                        )
                    else
                        null,
                    .example_spec_json = null,
                    .note = try allocator.dupe(u8, "Spec JSON: {\"query_id\":0,\"item_id\":\"0x0\",\"initiator\":\"0:...\",\"owner\":\"0:...\",\"data_comment\":\"<text>\",\"revoked_at\":0}"),
                };
            }
...
test "agent tools inspectContract builds templates for wallet discovery and nft static data messages" {
    const allocator = std.testing.allocator;

    const incoming_body = try @import("../contract/standard_body.zig").createJettonProvideWalletAddressMessage(
        allocator,
        31,
        "0:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
        true,
    ...
    const outgoing_body = try @import("../contract/standard_body.zig").createNftReportStaticDataMessage(
        allocator,
        32,
        &.{ 0x12, 0x34 },
    ...
    const FakeClient = struct {
        ...
        pub fn getTransactions(self: *@This(), addr: []const u8, limit: u32) ![]core_types.Transaction {
            ...
            errdefer in_body.deinit(self.allocator);
            in_msg.* = .{
                .hash = try self.allocator.dupe(u8, "inspect-discovery-in"),
                .source = try address_mod.parseAddress("0:9999999999999999999999999999999999999999999999999999999999999999"),
                .destination = try address_mod.parseAddress(addr),
                .value = 111,
                .body = in_body,
            ...
            errdefer out_body.deinit(self.allocator);
            out_msg.* = .{
                .hash = try self.allocator.dupe(u8, "inspect-discovery-out"),
                .source = try address_mod.parseAddress(addr),
                .destination = try address_mod.parseAddress("0:BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB"),
                .value = 22,
                .body = out_body,
            ...
            txs[0] = .{
                .hash = try self.allocator.dupe(u8, "inspect-discovery-tx"),
                .lt = 77,
                .timestamp = 88,
                .in_msg = in_msg,
                .out_msgs = out_msgs,
    ...
    try std.testing.expect(result.success);
    try std.testing.expectEqualStrings("jetton_provide_wallet_address", result.observed_messages[0].opcode_name.?);
    try std.testing.expectEqualStrings(
        "ton-zig-agent-kit cell build-standard jetton_provide_wallet_address @spec.json",
        result.observed_messages[0].template.?.body_cli_template.?,
    );
    try std.testing.expect(result.observed_messages[0].template.?.example_spec_json != null);
    try std.testing.expect(std.mem.indexOf(u8, result.observed_messages[0].template.?.example_spec_json.?, "\"include_address\":true") != null);

    try std.testing.expectEqualStrings("nft_report_static_data", result.observed_messages[1].opcode_name.?);
    try std.testing.expectEqualStrings(
        "ton-zig-agent-kit cell build-standard nft_report_static_data @spec.json",
        result.observed_messages[1].template.?.body_cli_template.?,
    );
    try std.testing.expect(result.observed_messages[1].template.?.example_spec_json != null);
    try std.testing.expect(std.mem.indexOf(u8, result.observed_messages[1].template.?.example_spec_json.?, "\"index\":\"0x1234\"") != null);
}

test "agent tools inspectContract builds templates for nft royalty messages" {
    const allocator = std.testing.allocator;

    const incoming_body = try @import("../contract/standard_body.zig").createNftGetRoyaltyParamsMessage(allocator, 41);
    defer allocator.free(incoming_body);

    const outgoing_body = try @import("../contract/standard_body.zig").createNftReportRoyaltyParamsMessage(
        allocator,
        42,
        25,
        1000,
    ...
    const FakeClient = struct {
        ...
        pub fn getTransactions(self: *@This(), addr: []const u8, limit: u32) ![]core_types.Transaction {
            ...
            errdefer in_body.deinit(self.allocator);
            in_msg.* = .{
                .hash = try self.allocator.dupe(u8, "inspect-royalty-in"),
                .source = try address_mod.parseAddress("0:9999999999999999999999999999999999999999999999999999999999999999"),
                .destination = try address_mod.parseAddress(addr),
                .value = 111,
                .body = in_body,
            ...
            errdefer out_body.deinit(self.allocator);
            out_msg.* = .{
                .hash = try self.allocator.dupe(u8, "inspect-royalty-out"),
                .source = try address_mod.parseAddress(addr),
                .destination = try address_mod.parseAddress("0:BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB"),
                .value = 22,
                .body = out_body,
            ...
            txs[0] = .{
                .hash = try self.allocator.dupe(u8, "inspect-royalty-tx"),
                .lt = 77,
                .timestamp = 88,
                .in_msg = in_msg,
                .out_msgs = out_msgs,
    ...
    try std.testing.expect(result.success);
    try std.testing.expectEqualStrings("nft_get_royalty_params", result.observed_messages[0].opcode_name.?);
    try std.testing.expectEqualStrings(
        "ton-zig-agent-kit cell build-standard nft_get_royalty_params @spec.json",
        result.observed_messages[0].template.?.body_cli_template.?,
    );
    try std.testing.expect(result.observed_messages[0].template.?.example_spec_json != null);
    try std.testing.expect(std.mem.indexOf(u8, result.observed_messages[0].template.?.example_spec_json.?, "\"query_id\":41") != null);

    try std.testing.expectEqualStrings("nft_report_royalty_params", result.observed_messages[1].opcode_name.?);
    try std.testing.expectEqualStrings(
        "ton-zig-agent-kit cell build-standard nft_report_royalty_params @spec.json",
        result.observed_messages[1].template.?.body_cli_template.?,
    );
    try std.testing.expect(result.observed_messages[1].template.?.example_spec_json != null);
    try std.testing.expect(std.mem.indexOf(u8, result.observed_messages[1].template.?.example_spec_json.?, "\"numerator\":25") != null);
    try std.testing.expect(std.mem.indexOf(u8, result.observed_messages[1].template.?.example_spec_json.?, "\"denominator\":1000") != null);
}

test "agent tools inspectContract builds templates for sbt messages" {
    const allocator = std.testing.allocator;
    ...
    const incoming_body = try @import("../contract/standard_body.zig").createSbtProveOwnershipMessage(
        allocator,
        51,
        "0:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
        forward_payload,
    ...
    const outgoing_body = try @import("../contract/standard_body.zig").createSbtOwnerInfoMessage(
        allocator,
        52,
        &.{ 0x12, 0x34 },
        "0:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
    ...
    const FakeClient = struct {
        ...
        pub fn getTransactions(self: *@This(), addr: []const u8, limit: u32) ![]core_types.Transaction {
            ...
            errdefer in_body.deinit(self.allocator);
            in_msg.* = .{
                .hash = try self.allocator.dupe(u8, "inspect-sbt-in"),
                .source = try address_mod.parseAddress("0:9999999999999999999999999999999999999999999999999999999999999999"),
                .destination = try address_mod.parseAddress(addr),
                .value = 111,
                .body = in_body,
            ...
            errdefer out_body.deinit(self.allocator);
            out_msg.* = .{
                .hash = try self.allocator.dupe(u8, "inspect-sbt-out"),
                .source = try address_mod.parseAddress(addr),
                .destination = try address_mod.parseAddress("0:BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB"),
                .value = 22,
                .body = out_body,
            ...
            txs[0] = .{
                .hash = try self.allocator.dupe(u8, "inspect-sbt-tx"),
                .lt = 77,
                .timestamp = 88,
                .in_msg = in_msg,
                .out_msgs = out_msgs,
    ...
    try std.testing.expect(result.success);
    try std.testing.expectEqualStrings("sbt_prove_ownership", result.observed_messages[0].opcode_name.?);
    try std.testing.expectEqualStrings(
        "ton-zig-agent-kit cell build-standard sbt_prove_ownership @spec.json",
        result.observed_messages[0].template.?.body_cli_template.?,
    );
    try std.testing.expect(result.observed_messages[0].template.?.example_spec_json != null);
    try std.testing.expect(std.mem.indexOf(u8, result.observed_messages[0].template.?.example_spec_json.?, "\"with_content\":true") != null);

    try std.testing.expectEqualStrings("sbt_owner_info", result.observed_messages[1].opcode_name.?);
    try std.testing.expectEqualStrings(
        "ton-zig-agent-kit cell build-standard sbt_owner_info @spec.json",
        result.observed_messages[1].template.?.body_cli_template.?,
    );
    try std.testing.expect(result.observed_messages[1].template.?.example_spec_json != null);
